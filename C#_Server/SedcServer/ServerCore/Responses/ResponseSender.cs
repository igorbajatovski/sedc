...
namespace ServerCore.Responses
{
    internal static class ResponseSender
    {
        public static void Send(this Response response, Socket socket, WebServerOptions serverOptions)
        {
            ...
            {
                var constantHeaders = 
$@"HTTP/1.1 {response.ResponseCode.GetValue()} {response.ResponseCode.GetMessage()}
...
";
            ...
            else if(response.ResponseCode == ResponseCode.RedirectFound)
            {
                var responseMessage = 
$@"HTTP/1.1 {response.ResponseCode.GetValue()} {response.ResponseCode.GetMessage()}
Server: {serverOptions.ServerName}
...
{response.Body}
";
                messageBytes = Encoding.ASCII.GetBytes(responseMessage);
            }
