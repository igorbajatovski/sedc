    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            */
            ...
            var songsWithA = Songs.Where(song => song.Name.ToLower().StartsWith("a")).Select(song => new { Name = song.Name });
            Console.WriteLine("Songs that start with letter 'a': \n" + string.Join(",", songsWithA));
            Console.WriteLine();

            //2 - how many artists end with letter 'a' (case insensitive)
            var artistEndWithA = Artists.Where(artist => artist.FullName.ToLower().EndsWith("a")).Select(a => new { ArtistName = a.FullName });
            Console.WriteLine("Artist that ends with letter 'a': \n" + string.Join(",", artistEndWithA));
            Console.WriteLine();

            //3 - whats the name of the song with longest duration
            int maxDuration = Songs.Max(song => song.Duration);
            var longestSong = Songs.Where(song => song.Duration == maxDuration).Select(song => new { Name = song.Name, Duration = song.Duration });
            Console.WriteLine("Song with longest duration: \n" + string.Join(",", longestSong));
            Console.WriteLine();

            //4 - whats the total Duration of all Songs
            int totalDurationAllSongs = Songs.Sum(song => song.Duration);
            Console.WriteLine("Total Duration of all Songs: " + string.Join(",", totalDurationAllSongs));
            Console.WriteLine();

            //5 - how many albums have Songs longer than 300 seconds
            var albumsWithSongsLongerThenTHS = Albums.Where(album => album.Songs.Where(s => s.Duration > 300).ToArray().Count() > 0).
                                                Select(s => new { s.Name });
            Console.WriteLine("Albums that have Songs longer than 300 seconds: \n" + string.Join(",", albumsWithSongsLongerThenTHS));
            Console.WriteLine();
            ...
            var popRock = Artists.Where(a => a.Albums.Count(al => al.Genre == Genre.PopRock) > 1).Select(a =>  a.FullName);
            Console.WriteLine("Artists that have more than one album of PopRock genre: \n" + string.Join(" -- ", popRock));
            Console.WriteLine();

            var popRock1 = (from a in Artists
                           join al in Albums
                           on a.Id equals al.ArtistId
                           where al.Genre == Genre.PopRock
                           group a by a.FullName into g
                           select new { Artist = g.Key, Count = g.Count() > 1 }).Where(x => x.Count).Select(x => x.Artist);
            Console.WriteLine("Artists that have more than one album of PopRock genre: \n" + string.Join(" -- ", popRock1));
            Console.WriteLine();

            //7 - print the name of the album that has highest Average duration of a song
            var highestAverage = Albums.Select(a => a.Songs.Average(s => s.Duration)).Max();

            var averages = Albums.Select(a => a.Songs.Average(s => s.Duration));
            Console.WriteLine("Album avarage duration: \n" + string.Join(", ", averages));
            Console.WriteLine();

            var albumWithHighestAverage = Albums.Where(al => al.Songs.Average(s => s.Duration) == highestAverage).
                                            Select(r => new { Album = r.Name, AverageDuration = r.Songs.Average(s=> s.Duration) });
            Console.WriteLine("Album that has highest Average duration of a song: \n" + string.Join(" -- ", albumWithHighestAverage));
            Console.WriteLine();

            //8 - how many characters has the song that has the shortest Duration
            var songMinDuration = Songs.Min(s => s.Duration);
            var characters = Songs.Where(s => s.Duration == songMinDuration).Select(s => new { s.Name, Characters = s.Name.Length });
            Console.WriteLine("characters of the song that has the shortest Duration: \n" + string.Join(",", characters));
            Console.WriteLine();

            //9 - print the name and the genre of the album that has most songs
            int max = Albums.Select(a => new { a.Name, Count = a.Songs.Count }).Max(r => r.Count);
            var album1 = Albums.Where(a => a.Songs.Count == max).Select(r => new { r.Name, r.Genre });
            Console.WriteLine("name and the genre of the album that has most songs: \n" + string.Join(",", album1));
            Console.WriteLine();

            var albumWithMostSongsCount = (from al in Albums
                                           join song in Songs
                                           on al.Id equals song.AlbumId
                                           group al by al.Name into g
                                           select new { g.Key, CountSongs = g.Count() }).Max(r => r.CountSongs);
            var albumWithMostSongs = Albums.Where(a => a.Songs.Count == albumWithMostSongsCount).Select(r => new { r.Name, r.Genre });
            Console.WriteLine("name and the genre of the album that has most songs: \n" + string.Join(",", albumWithMostSongs));
            Console.WriteLine();

            //10 - print the name of the artist that has most songs
            var totalSongs = Artists.Join(Albums, ar => ar.Id, al => al.ArtistId, (ar, al) => new { ar, al }).
                            Join(Songs, r => r.al.Id, s => s.AlbumId, (r, s) => new { r.ar, r.al, s } ).GroupBy(r => r.ar.FullName).
                                Select(g => new { Artist = g.Key, TotalSongs = g.Count() });

            int maxTotalSongs = totalSongs.Max(r => r.TotalSongs);
            var artist1 = totalSongs.Where(r => r.TotalSongs == maxTotalSongs);

            Console.WriteLine("name of the artist that has most songs: \n" + string.Join(",", artist1));
            Console.WriteLine();

            //11 - print the type of the artist(SoloArtist/Band) that has most albums published before year 2000
            var albumsBefore2000 = from ar in Artists
                                      join al in Albums
                                      on ar.Id equals al.ArtistId
                                      where al.Year < 2000
                                      group ar by ar.FullName into g
                                      select new { Artist = g.Key, AlbumsBefore2000 = g.Count() };
                                      
            var maxAlbumsBefore2000 = albumsBefore2000.Max(r=>r.AlbumsBefore2000);
            string[] maxAlbumBefore2000 = albumsBefore2000.Where(r => r.AlbumsBefore2000 == maxAlbumsBefore2000).Select(r=>r.Artist).ToArray();
            var albumBefore2000 = Artists.Where(r => r.FullName == maxAlbumBefore2000[0]).Select(r=> new { r.FullName, r.ArtistType });
            Console.WriteLine("type of the artist(SoloArtist/Band) that has most albums published before year 2000: \n" + string.Join(",", albumBefore2000));
            Console.WriteLine();
